						...
						@foreach($results as $k=>$result)
							...
							<div class="search-result col-md-6 col-xs-12">
								<div class="panel panel-default" style="overflow:hidden">
									...
									<a href="{{ strpos($result['url'],'http') === false ? 'http://'.$result['url'] : $result['url'] }}" target="_blank" {!! $rel_ex !!}><h3 class="text-primary" style="margin:auto">{!! html_entity_decode(str_ireplace($q, '<b>'.$q.'</b>', $result['title'])) !!}</h3></a>
									...
									<h3>{{ !empty($findDollar) ? strtolower($findDollar[0]) : (!empty($findPercent) ? strtolower($findPercent[0]) : 'CODE') }}</h3>
										...
										@endif
										<p class="result-url">
											<a href="{{ strpos($result['url'],'http') === false ? 'http://'.$result['url'] : $result['url'] }}" target="_blank" {!! $rel_ex !!}>{{ str_limit(html_entity_decode($result['url']),80) }}
											<sup><span class="fa fa-external-link"></span></sup></a>
										</p>
									</div>
								</div>
							</div>
