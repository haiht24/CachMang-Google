...
<div class="panel panel-default" style="height:110px;overflow:hidden">
	...
	<strong>
	<a href="{{ $link=strpos($result['domain'],'http') === false ? 'http://'.$result['domain'] : $result['domain'] }}" target="_blank">{{ str_limit(html_entity_decode($result['title']),80) }}</a></strong>  <span class="fa fa-external-link"></span><br/>
		<p>
